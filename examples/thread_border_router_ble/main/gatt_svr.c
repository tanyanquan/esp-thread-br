...
/* Custom 128-bit UUID for the GATT service */
...
static int gatt_svc_access(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    case BLE_GATT_ACCESS_OP_WRITE_CHR:
        if (conn_handle != BLE_HS_CONN_HANDLE_NONE) {
            ...
            ESP_LOGD(TAG, "Characteristic write by NimBLE stack; attr_handle=%d", attr_handle);
        }
        uuid = ctxt->chr->uuid;
        if (attr_handle == gatt_svr_chr_val_handle) {
    ...
    case BLE_GATT_ACCESS_OP_READ_DSC:
        if (conn_handle != BLE_HS_CONN_HANDLE_NONE) {
            ESP_LOGD(TAG, "Descriptor read; conn_handle=%d attr_handle=%d", conn_handle, attr_handle);
        } else {
            ESP_LOGD(TAG, "Descriptor read by NimBLE stack; attr_handle=%d", attr_handle);
        }
        uuid = ctxt->dsc->uuid;
...
void gatt_svr_register_cb(struct ble_gatt_register_ctxt *ctxt, void *arg)
{
    ...
    case BLE_GATT_REGISTER_OP_SVC:
        ESP_LOGD(TAG, "Registered service %s with handle=%d", ble_uuid_to_str(ctxt->svc.svc_def->uuid, buf),
                 ctxt->svc.handle);
        break;
    ...
    case BLE_GATT_REGISTER_OP_DSC:
        ESP_LOGD(TAG, "Registering descriptor %s with handle=%d", ble_uuid_to_str(ctxt->dsc.dsc_def->uuid, buf),
                 ctxt->dsc.handle);
        break;
